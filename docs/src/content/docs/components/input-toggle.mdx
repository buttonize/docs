---
title: Input.toggle
description: Input.toggle displays a checkbox.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'
import ComponentDemo from '@components/ComponentDemo.astro'
import FrameworkSelect from '@components/FrameworkSelect.astro'
import FrameworkContent from '@components/FrameworkContent.astro'
import PropDescription from '@components/PropDescription.astro'
import PropMarker from '@components/PropMarker.astro'

import SpacingProps from '../../../components/SpacingProps.mdx'
import SizingProps from '../../../components/SizingProps.mdx'

<FrameworkSelect>
	<FrameworkContent>
		<Fragment slot="cdk">
			[Source](https://github.com/buttonize/buttonize/blob/master/cdk/components/input/toggle.ts)
		</Fragment>
	</FrameworkContent>
</FrameworkSelect>

Displays an interactable checkbox within a Buttonize page.

```ansi frame="none"
[38;5;39mInput.toggle[0m(props)
```

## Usage

<FrameworkContent>
   <Fragment slot="cdk">
    ```ts title="lib/MyStack.ts" collapse={1-5}
    import { ButtonizeApp, Input } from '@buttonize/cdk'

    new ButtonizeApp(this, 'DemoApp', {
      name: 'Demo App'
    }).page('DemoPage', {
      body: [
        Input.toggle({
          id: 'toggleSwitch', // the name of the variable in the runtime state
          label: 'Enable Feature',
        }),
        Display.text("toggleSwitch's value: {{toggleSwitch}}"),
      ]
    })
    ```

   </Fragment>
</FrameworkContent>

## Preview

<ComponentDemo
	components={[
		{
			typeName: 'input.toggle',
			props: {
				id: 'toggleSwitch',
				label: 'Enable Feature'
			}
		},
		{
			typeName: 'display.text',
			props: {
				label: "toggleSwitch's value: {{toggleSwitch}}"
			}
		}
	]}
/>

## Runtime State

The value in the runtime state is the boolean value of the toggle input.

```ts title="lib/MyStack.ts" "'name'"
Input.toggle({
	id: 'bool',
	label: 'My Checkbox'
})
```

```json title="Runtime state" frame="terminal" ""name""
{
	"bool": true
}
```

As demonstrated by the Usage example above, you can access variables from the Runtime State
via the "\{\{bool\}\}" syntax from elsewhere in your Buttonize app.

## Props

<FrameworkContent>
	<Fragment slot="cdk">
		<PropDescription>
			<Fragment slot="title">
				{/* keep-good-prettier-formatting */}
				### id
			</Fragment>
			<PropMarker slot="markers" type="required" />
			<Fragment slot="type">string</Fragment>
			<Fragment slot="description">
				The name of the variable containing the selected option. This variable
				can then be referenced elsewhere in your Buttonize app.
			</Fragment>
		</PropDescription>
		<PropDescription>
			<Fragment slot="title">
				{/* keep-good-prettier-formatting */}
				### label
			</Fragment>
			<PropMarker slot="markers" type="optional" />
			<Fragment slot="type">string</Fragment>
			<Fragment slot="description">
				The text displayed by the checkbox.
			</Fragment>
		</PropDescription>
		<PropDescription>
			<Fragment slot="title">
				{/* keep-good-prettier-formatting */}
				### initialValue
			</Fragment>
			<PropMarker slot="markers" type="optional" />
			<Fragment slot="type">boolean</Fragment>
			<Fragment slot="description">
				The initial state of the checkbox. Default is `false`.
			</Fragment>
		</PropDescription>
		<PropDescription>
			<Fragment slot="title">
				{/* keep-good-prettier-formatting */}
				### disabled
			</Fragment>
			<PropMarker slot="markers" type="optional" />
			<Fragment slot="type">boolean</Fragment>
			<Fragment slot="description">
				Whether or not the checkbox is disable. Default is `false`.
			</Fragment>
		</PropDescription>
		<SpacingProps />
		<SizingProps />
	</Fragment>
</FrameworkContent>
