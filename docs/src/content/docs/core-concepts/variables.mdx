...
Every application has some kind of [runtime state](/core-concepts/runtime-state/) and Buttonize apps are no exception.

When an app is loaded, it starts with an empty state. Nnce the browser tab is closed or refreshed, the state is destroyed.

Every `Input` stores some data in the runtime state. You can learn more in the "Runtime State" section of each component.

You can access the state value in your app by using the `{{variable_name}}` notation in your code.

## Example

<FrameworkContent>
  <Fragment slot="cdk">
    ```ts title="lib/MyStack.ts" collapse={1-7} {"'name' is used as a key when storing the input value in the state":12-13} {"The input value is displayed here by using {{name}}":17-18}
...
The contents of the variable notation are evaluated via [JMESPath](https://jmespath.org). It's like JSONPath but more powerful.

So you can use simple notation like `{{myVariable}}`, or you can do more advanced stuff like what's shown below.
...
<FrameworkContent>
  <Fragment slot="cdk">
    ```ts title="lib/MyStack.ts" collapse={1-7} {"\"name\"'s value is read from \"SecondPage\" here": 17-18}  {"\"name\" value is read from \"FirstPage\" here": 28-29}
    import { Action, Buttonize, ButtonizeApp, Display, Input } from '@buttonize/cdk'
...
This can be useful when you need to pre-fetch data that will be displayed to the user, such as dynamic options for a select box.
