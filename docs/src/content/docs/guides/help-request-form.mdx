...
Within each page's `body` prop, we then fill in our frontend! It's here we layout what our app should look like and which actions
certain components should take when interacted with.
...
second page to make things more clear
...
This component renders text on the screen and interpolates `{{email}}` using the Runtime State, populated by the `Input.text`
...
Notice that Runtime State values aren't just used for display purposes. They're referenced within the payload with which we invoke the lambda
...
construct referencing some lambda you've created yourself. This way, your Buttonize app can drop smoothly into your CDK stack and play nice
...
There's not much to this one. It exists solely to give feedback to the user, but the flow could go on if so desired.
